...
복원도 90% 달성 후 추억 사진이 열리는 순간 약 0.7초의 밝은 3음 상승 벨소리를 정확히 한 번 재생한다. 소리는 짧은 잔향을 가진 `띠-리-링` 느낌으로 코드에서 파형을 생성하며 마우스·키보드·헤드셋이 공유한다. `전체 음량`과 `효과음` 설정을 따르고, 완료 뒤 추가 진행도 신호가 와도 중복 재생하지 않는다. 외부 음원 파일을 사용하지 않으므로 별도 제3자 라이선스는 없다.
...
- PlayMode: 각 스테이지 완료 시 추억 패널과 3음 벨소리가 한 번만 발생하고 후속 진행도 갱신에는 반복되지 않는지 검사한다.
