...
## 2026-08-16 — 상호작용 전용 하이라이트 참조 보존

**문제와 수정**

- 추가 검토에서 직접 하이라이트 참조 없이 `StageInputController.Configure`를 호출하면, 이미 `StageInteractionController`에 설정된 유효한 하이라이트를 `null`로 덮어쓰는 문제를 확인했다.
- 직접 참조가 있을 때만 상호작용 컨트롤러에 동기화하도록 바꿔, 직접 참조 우선과 상호작용 전용 대체 경로를 모두 유지했다.

**TDD와 검증**

- RED: 상호작용 컨트롤러에만 `HighlightController`를 설정하고 실제 가상 Keyboard `Space` 이벤트를 보내면 표면 `_HighlightPulse`가 `0`으로 남았다. PlayMode는 통과 12개, 실패 1개였다.
- GREEN: 참조 보존 후 같은 장치 입력 회귀 테스트와 전체 PlayMode 13개가 통과했다.
...
## 2026-08-16 — 스테이지 완료 UI·오디오 잠금 회귀 수정

**문제와 원인**

- 오디오가 `StageInputController`의 원시 좌클릭 신호를 먼저 받아 UI 위에서 누른 경우에도 루프를 시작할 수 있었다.
- 완료 시 장비 입력은 꺼졌지만 도구 버튼 콜백은 살아 있어 선택 모델과 오디오를 다시 바꿀 수 있었다.
- 진행 모델은 90% 교차 값을 알렸지만 완료 이벤트가 동기 실행되어 휠이 이전 표시값에서 애니메이션을 시작했고, 89.9% 숫자는 반올림되어 미리 90%로 보였다.

**수정**

- 입력 컨트롤러가 UI 위 여부를 좌클릭 변경 이벤트보다 먼저 계산해 노출하고, 스테이지가 UI 입력이면 도구 루프를 시작하지 않거나 즉시 멈추도록 했다.
- `ToolSelectorView.SetInteractable(false)`와 콜백 내부 잠금 가드를 함께 적용해 완료 후 직접 콜백 호출도 선택을 바꾸지 못하게 했다.
- 완료 교차 값을 먼저 휠에 표시하고 다음 프레임부터 0.35초 애니메이션을 진행하며, 정수 표시는 내림으로 계산해 89.9%를 89%로 유지했다.

**TDD와 검증**

- RED: `UpdateCleaningAudio`, 도구 UI 잠금 상태와 확장된 테스트 구성 계약이 없어 Unity 컴파일 오류 5건을 확인했다.
- 첫 GREEN 실행에서 Unity의 `StartCoroutine`이 첫 `yield` 전까지 즉시 실행되어 교차값 0.9가 같은 호출 안에서 0.90068로 덮이는 실패 1개를 확인하고, 애니메이션을 다음 프레임부터 시작하도록 수정했다.
